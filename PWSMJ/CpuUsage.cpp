...
CpuUsage::CpuUsage(void) : m_nCpuUsage(-1), m_dwLastRun(0), m_lRunCount(0), m_nCpuTimeDiff(0), m_nCpuTimeTotal(0)
{
		...
		*/
		...
		if (!IsFirstRun())
		{
			...
			*/
			...
			if ( FileTimeToSystemTime( &ftProcUser, &userSystemTime ) != -1 )
			{
				m_nCpuTime[1] = (double)userSystemTime.wHour * 3600.0 +
					(double)userSystemTime.wMinute * 60.0 +
					(double)userSystemTime.wSecond +
					(double)userSystemTime.wMilliseconds / 1000.0;
			}

			m_nCpuTimeDiff = m_nCpuTime[1] - m_nCpuTime[0];
		} 
		...
		//nCpuUsageCopy = m_nCpuUsage;
		m_nCpuTime[0] = m_nCpuTime[1];
		m_nCpuTimeTotal+=m_nCpuTimeDiff;
	}

	::InterlockedDecrement(&m_lRunCount); 
}

short CpuUsage::GetCpuUsage()
{
	return m_nCpuUsage;
}

double CpuUsage::GetCpuTime()
{
	return m_nCpuTimeDiff;
}

double CpuUsage::GetTotalCpuTime()
{
	return m_nCpuTimeTotal;
}
